...
test('ResultsDisplay is built around side-by-side original and canonical panes with opt-in controls', async () => {
  ...
  assert.match(source, /Review status, glossary coverage, locale readiness, and a direct comparison with a baseline result\./)
  assert.match(source, /Reviewer status/)
  assert.match(source, /Glossary coverage/)
  assert.match(source, /Locale readiness/)
  assert.match(source, /Baseline comparison/)
  assert.match(source, /Run baseline comparison/)
  assert.match(source, /Canonical vs baseline/)
  assert.match(source, /Current canonical result/)
  assert.match(source, /Baseline result without glossary hints/)
  assert.match(source, /What changed/)
  assert.match(source, /Baseline uses the same request without glossary hints\./)
